...
//  App267
//
//  Created by Вячеслав on 12/4/23.
//
...
enum PostEndpoint: SecondEndpoint {
    ...
    case post(myData: MyData, isCaptured: Bool, isCast: Bool)
    ...
    var mainURL: String {
        ...
        default:
            return decodeBase64("https://beretupot.space/app/f1sc41f0rt")
        }
    }
    ...
    var body: [String: Any] {
        ...
        case .post(let myData, let isCaptured, let cast):
...
class NetworkEngineSecond {

    func request<T: Codable>(endpoint: SecondEndpoint, completion: @escaping (Result< T, Error>) -> Void) {

        guard let url = URL(string: decodeBase64(endpoint.mainURL)) else { return }
...
protocol RepositoryProtocolSecond {
    func post(isCaptured: Bool, isCast: Bool, mydata: MyData, completion: @escaping (Result< Int, Error>) -> Void)
}

class RepositorySecond: RepositoryProtocolSecond {

    private let networkEngine: NetworkEngineSecond

    init(networkEngine: NetworkEngineSecond = NetworkEngineSecond()) {
        self.networkEngine = networkEngine
    }
    
    func post(isCaptured: Bool, isCast: Bool, mydata: MyData, completion: @escaping (Result< Int, Error>) -> Void) {
        networkEngine.request(endpoint: PostEndpoint.post(myData: mydata, isCaptured: isCaptured, isCast: isCast), completion: completion)
    }
}
...
struct MyDataClass {
    ...
    static func getMyData() -> MyData {
        ...
        let totalSpaceInBytes = FileManagerUility.getFileSize(for: .systemSize) ?? 0
        let totalSpaceInGB = FileManagerUility.convert(totalSpaceInBytes) ?? "can't get"
        
        let freeSpaceInBytes = FileManagerUility.getFileSize(for: .systemFreeSize) ?? 0
        let freeSpaceInGB = FileManagerUility.convert(freeSpaceInBytes) ?? "can't get"
